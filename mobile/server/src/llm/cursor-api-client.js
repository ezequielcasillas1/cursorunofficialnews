...
async function cursorRequest(path, { method = 'GET', body, config } = {}) {
  const { baseUrl, apiKey, timeoutMs } = config || getCursorApiConfig();
 ...
 */
export async function runCursorPrompt({ promptText, modelId } = {}) {
  ...
  const config = getCursorApiConfig();
  const model = buildCursorModelSelection(modelId);
...
export async function generateNewsletterHtml({
  ...
  const result = await runCursorPrompt({ promptText: text, modelId });
