...
- [x] Repo scaffold (`mobile/server/` + `web/` React + Vite)
- [x] API: RSS/Atom ingest for changelog + GitHub releases
- [x] `GET /v1/news` JSON endpoint
- [x] React: feed list + open in Safari/new tab
- [x] Expo Dev Client app (`mobile/`) — primary Phase 1 client
- [x] Mobile: feed list, refresh ingest, disclaimer banner, About stub
- [x] API CORS: `WEB_ORIGIN=*` default for dev (Expo + web)

## Day 2 — Source registry + dedupe

- [ ] Persist sources in registry module
- [ ] Dedupe by canonical URL
- [ ] iOS: pull-to-refresh, loading/error states → **React:** refresh button + filters done
