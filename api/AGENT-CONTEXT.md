...
| **API workspace** | `C:\Dev\CursorAINews\api` (this folder — onboarding docs live here) |
...
| PasteCraft `request.md` / `refresh.md` | `CURSOR-AI-NEWS-PHASE-PLAN.md` (this folder) |
...
|---|---|---|
| API | `C:\Dev\CursorAINews\api\` | Express `:8787` — ingest, normalize, JSON |
| Mobile | `C:\Dev\CursorAINews\mobile\` | Expo Dev Client — **primary client** |
| Web | `C:\Dev\CursorAINews\web\` | React + Vite — legacy preview |
...
When workspace is **`C:\Dev\CursorAINews\api`**, read files in **this folder**:

1. **[CURSOR-AI-NEWS-PHASE-PLAN.md](CURSOR-AI-NEWS-PHASE-PLAN.md)** — scope, verify steps, status checkboxes
2. **[unofficial-cursor-news-feed-service.mdc](.cursor/rules/unofficial-cursor-news-feed-service.mdc)** — phases 1–7 detail
3. **[cursor-ai-news-project-root.mdc](.cursor/rules/cursor-ai-news-project-root.mdc)** — workspace routing (alwaysApply)

Canonical copies also live at `C:\Dev\CursorAINews\docs\` and `C:\Dev\CursorAINews\.cursor\rules\`.
...
| Item | Status |
|---|---|
| Changelog RSS URL | Done → `https://cursor.com/changelog/rss.xml` (`src/sources/registry.js`) |
| API ingest (~50 changelog items) | Done — bootstrap on startup + `POST /v1/ingest` |
| Zscaler TLS | Done (`node --use-system-ca` in `package.json` `dev`/`start`) |
...
| Item | Status |
|---|---|
| `dedupeNewsItems` by canonical URL | Done — `src/normalize/news-item.js` |
...
| Scrape ingest path | Done — `src/ingest/scrape.js` (env-gated) |
