...
import { StatusBar } from '../components/AboutPanel.jsx';
...
import { getFeedPageMeta } from '../seo/pageMeta.js';

export function FeedPage({ categoryId = 'all' }) {
  const selectedCategory = categoryId;
  const pageMeta = useMemo(() => getFeedPageMeta(categoryId), [categoryId]);
  ...
  const loadNews = useCallback(async (catId, filter, page, searching) => {
    setError('');
    setLoading(true);
    try {
      ...
      const [news, meta] = await Promise.all([
        fetchNews({
          category: getCategoryApiParam(catId),
  ...
  return (
    <AppShell onRefresh={handleRefresh} refreshing={refreshing}>
      <PageMeta
