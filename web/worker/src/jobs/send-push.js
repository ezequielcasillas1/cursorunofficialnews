import { assemblePushForItems } from '../shared/notifications/assemble-push.js';
...
async function handleTicketErrors(db, messages, tickets) {
  let failed = 0;
  const errors = [];
  ...
  for (let i = 0; i < tickets.length; i += 1) {
    ...
    if (DEAD_TOKEN_ERRORS.has(detail) && message?.to) {
      await unregisterDevice(db, message.to);
 ...
 */
export async function notifySubscribers(db, newItems, env) {
  if (env?.PUSH_NOTIFICATIONS === 'false') {
  ...
  for (const device of await listDevices(db)) {
  ...
  try {
    ...
    for (const chunk of chunks) {
      ...
      sent += chunkSent;
      const { failed: chunkFailed, errors } = await handleTicketErrors(db, chunk, tickets);
      failed += chunkFailed;
