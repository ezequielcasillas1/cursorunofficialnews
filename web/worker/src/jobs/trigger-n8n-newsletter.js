...
/**
 * Resolve the active n8n webhook URL.
 * Priority: explicit N8N_NEWSLETTER_WEBHOOK_URL override, then target-specific URL.
 * Target defaults to `test` in development and `prod` in production.
 */
export function resolveN8nWebhookUrl(env) {
  const override = env?.N8N_NEWSLETTER_WEBHOOK_URL?.trim();
  if (override) return override;

  const configuredTarget = env?.N8N_NEWSLETTER_TARGET?.trim().toLowerCase();
  const target =
    configuredTarget === 'test' || configuredTarget === 'prod'
      ? configuredTarget
  ...
  if (target === 'prod') {
    return env?.N8N_NEWSLETTER_WEBHOOK_URL_PROD?.trim() || '';
  }

  return env?.N8N_NEWSLETTER_WEBHOOK_URL_TEST?.trim() || '';
}

export function getN8nNewsletterTarget(env) {
  const override = env?.N8N_NEWSLETTER_WEBHOOK_URL?.trim();
  if (override) return 'override';

  const configuredTarget = env?.N8N_NEWSLETTER_TARGET?.trim().toLowerCase();
  if (configuredTarget === 'test' || configuredTarget === 'prod') return configuredTarget;
  return env?.ENVIRONMENT === 'production' ? 'prod' : 'test';
}

export function isN8nNewsletterConfigured(env) {
  return Boolean(resolveN8nWebhookUrl(env));
}
...
export async function triggerN8nNewsletter({ newItems, ingestAt } = {}, env) {
  const webhookUrl = resolveN8nWebhookUrl(env);
  if (!webhookUrl) {
    return { skipped: true, reason: 'n8n webhook URL not configured' };
  }
  ...
  try {
    ...
    console.log(
      JSON.stringify({
        ...
        target: getN8nNewsletterTarget(env),
        newItems: payload.newItemCount,
        status: response.status,
      }),
    );

    return { triggered: true, status: response.status, target: getN8nNewsletterTarget(env) };
  } catch (error) {
    const message =
