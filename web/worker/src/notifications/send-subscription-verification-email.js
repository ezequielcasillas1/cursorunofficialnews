...
export function isSubscriptionVerificationEmailConfigured(env) {
  return isResendConfigured(env);
}

export function buildSubscriptionVerifyUrl(token, env) {
...
export async function sendSubscriptionVerificationEmail({ email, verificationToken }, env) {
  if (!isSubscriptionVerificationEmailConfigured(env)) {
    throw new Error('Email verification is unavailable right now.');
  }

  const resend = getResendClient(env);
  const from = getTransactionalFromAddress(env);
  const verifyUrl = buildSubscriptionVerifyUrl(verificationToken, env);
  const ttlMinutes = getVerificationTtlMinutes(env);
