...
describe('resolveMembershipClaim', () => {
  it('returns newsletter entitlement for whitelisted email without active Stripe membership', async () => {
    ...
    const claim = await resolveMembershipClaim(db, 'ezequielcasillas1@gmail.com', env);
    assert.ok(claim);
    assert.equal(claim.email, 'ezequielcasillas1@gmail.com');
    assert.equal(claim.newsletterUnlocked, true);
    assert.equal(claim.adFree, true);
    assert.ok(claim.membershipToken);
  });

  it('reactivates an inactive whitelisted row instead of leaving it stale', async () => {
    const rows = new Map([
      [
        'ezequielcasillas1@gmail.com',
        {
          email: 'ezequielcasillas1@gmail.com',
          membership_token: 'stale-token',
    ...
    const claim = await resolveMembershipClaim(db, 'ezequielcasillas1@gmail.com', env);
    assert.equal(claim?.membershipStatus, 'active');
    assert.equal(rows.get('ezequielcasillas1@gmail.com')?.active, 1);
  });
});
