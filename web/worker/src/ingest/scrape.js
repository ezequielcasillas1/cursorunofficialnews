...
export function isScrapeConfigured(env) {
  return Boolean(env?.SCRAPE_API_URL?.trim() && env?.SCRAPE_API_KEY?.trim());
}
...
async function callScrapeApi(pageUrl, env) {
  const apiUrl = env.SCRAPE_API_URL.trim();
  const apiKey = env.SCRAPE_API_KEY.trim();
...
async function fetchScrapeSource(source, env) {
  const pageUrls =
  ...
  for (const pageUrl of pageUrls) {
    ...
    if (isScrapeConfigured(env)) {
      ({ html, metadata } = await callScrapeApi(pageUrl, env));
    } else {
...
export async function ingestScrapeSources(env) {
  ...
  const batches = await Promise.all(
    sources.map(async (source) => {
      try {
        return await fetchScrapeSource(source, env);
      } catch (err) {
