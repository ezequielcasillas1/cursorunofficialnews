...
import { getMembershipClaimTtlMinutes } from '../store/membership-claim-requests.js';
import {
  getResendClient,
...
export function isMembershipClaimEmailConfigured(env) {
  return isResendConfigured(env);
}

export function buildMembershipClaimUrl(token, env) {
  const base = getPublicWebBase(env).replace(/\/$/, '');
  return `${base}/?membership_claim_token=${encodeURIComponent(token)}`;
}
...
export async function sendMembershipClaimEmail({ email, claimToken }, env) {
  if (!isMembershipClaimEmailConfigured(env)) {
    throw new Error('Membership email verification is unavailable right now.');
  }

  const resend = getResendClient(env);
  const from = getTransactionalFromAddress(env);
  const claimUrl = buildMembershipClaimUrl(claimToken, env);
  const ttlMinutes = getMembershipClaimTtlMinutes(env);

  const subject = 'Verify your Unofficial Cursor News membership';
...
<body style="margin:0;padding:0;background:#f0ebe3;">
        ...
        <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:560px;background:#fffdf9;border:1px solid #ddd6c8;border-radius:8px;">
          <tr>
            <td style="background:#0d1b2a;padding:28px 32px 24px;border-radius:8px 8px 0 0;">
              ...
              <h1 style="margin:0;color:#faf7f2;font-family:Georgia,'Times New Roman',serif;font-size:28px;font-weight:700;line-height:1.2;">Verify your membership</h1>
            </td>
          </tr>
          <tr>
            <td style="padding:28px 32px;">
              <p style="margin:0 0 16px;color:#1c1c24;font-family:Georgia,'Times New Roman',serif;font-size:16px;line-height:1.6;">
                If you requested to restore your membership for <strong>${escapeHtml(email)}</strong>, use the button below to verify control of this email and unlock ad-free browsing plus the email newsletter on this device.
              </p>
              <p style="margin:0 0 24px;color:#5c5c6a;font-family:Inter,Arial,sans-serif;font-size:13px;line-height:1.6;">
                This link expires in ${ttlMinutes} minutes.
              </p>
              <p style="margin:0 0 24px;">
                <a href="${escapeHtml(claimUrl)}" style="display:inline-block;background:#0d1b2a;border-radius:6px;color:#faf7f2;font-family:Inter,Arial,sans-serif;font-size:14px;font-weight:600;padding:12px 18px;text-decoration:none;">Verify and restore membership</a>
              </p>
  ...
  const text = [
    ...
    '',
    `If you requested to restore your membership for ${email}, verify it here:`,
    claimUrl,
    '',
    `This link expires in ${ttlMinutes} minutes.`,
    'If you did not request this, you can ignore this email.',
